...
use crate::{gameboard::GameBoard, piece_factory::{PieceFactory, PieceType}, traits::{Move, FallError, RotateError}};
...
impl Move for Piece{
    ...
    fn try_rotate(&mut self, board: &GameBoard) -> Result<(), RotateError> {
        board.can_rotate(self).then(|| {
            for square in self.squares.iter_mut() {
                square.position = square.get_rotated_position(&self.rotation);
            }
        }).ok_or(RotateError)
    }
}
